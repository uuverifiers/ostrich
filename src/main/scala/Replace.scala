...
object Replace {
  ...
  private def concatenation(lAfa: AFA, rAfa: AFA): AFA = {
    ...
    val finalStateSet =
      if(rAfa.finalStateSet(0))
        lAfa.finalStateSet ++ reAfa.finalStateSet
      else
        reAfa.finalStateSet
    val finalStates: AFormula = 
      (states.indices.toSet -- finalStateSet).foldLeft[AFormula](AFTrue) (
        _ & ~AFStateVar(_)
      )
  ...
  private def product(aut1: Automaton, aut2: Automaton): AFA = {
    val afa1: AFA = brics2AFATransducer(aut1, var1, var2)
    val afa2: AFA = brics2AFATransducer(aut2, var2, var1)
  ...
  private def buildReplaceAut(pattern: Automaton, 
                              substitution: Automaton): AFA = {
    pattern.getAcceptStates.asScala.foreach(_.getTransitions.clear())
    pattern.determinize()
    pattern.minimize()

    val symbolIdentity = buildSymbolIdentity(var1, var2)
    val prdct = product(pattern, substitution)
    var states: Vector[AFormula] = Vector.empty
    prdct.states.zipWithIndex foreach { case (state, ind) => 
      states :+= (
        if(state == AFFalse)
          AFStateVar(ind) & ~AFSpecSymb(var1) & ~AFSpecSymb(var2) & symbolIdentity
        else state)
    }

    new AFA(prdct.initialStates, states, prdct.finalStates)
  }

  def apply(pattern: String, substitution: String): AFA = {
    val autPattern: Automaton = buildAutomaton(pattern)
    val autSubstit: Automaton = buildAutomaton(substitution)
    val bm = buildBeginMarker(pattern)

    val repl = 
      buildReplaceAut(autPattern, 
                      autSubstit) shiftStateVariables bm.states.size - 1
    var states: Vector[AFormula] = Vector.empty

    bm.states foreach { state => 
      states :+= (if(state == AFFalse) repl.states.head else state)
    }

    states ++= repl.states.tail
    val finalFormula = repl.finalStates & bm.finalStates

    new AFA(bm.initialStates, states, finalFormula)
  }
