...
object CEBasicOperations {
  ...
  private def partitionStatesByVec(
    ...
    while (newDistinguishedPairs.nonEmpty) {
      ...
      val tmpPairs = newDistinguishedPairs
  ...
  def minimizeHopcroftByVec(
    ...
    val afterRemovingDeadStates = removeDeadState(afterDetermine)
    val s2equal = partitionStatesByVec(afterRemovingDeadStates)
    val ceAut = new CostEnrichedAutomaton
    ceAut.initialState = s2equal(afterRemovingDeadStates.initialState)
    for ((s, l, t, v) <- afterRemovingDeadStates.transitionsWithVec)
      ceAut.addTransition(s2equal(s), l, s2equal(t), v)
    for (s <- afterRemovingDeadStates.acceptingStates)
      ceAut.setAccept(s2equal(s), true)
    ceAut.regsRelation = afterRemovingDeadStates.regsRelation
    ceAut.registers = afterRemovingDeadStates.registers
    ceAut
  }
  // ---------------------------------------------------------------------------------------------------------------------
  ...
  def minimizeHopcroft(aut: CostEnrichedAutomatonBase) = {
    ParikhUtil.log(
      "CEBasicOperations.minimizeHopcroft: minimize automata by hopcroft algorithm"
    )
  ...
  def removeDuplicatedTrans(
    ...
    ParikhUtil.log(
      "CEBasicOperations.removeDuplicatedTrans: remove duplicated transitions"
    )
    if (aut.registers.isEmpty) return aut
    val ceAut = new CostEnrichedAutomaton
    val old2new = aut.states.map(s => (s, ceAut.newState())).toMap
