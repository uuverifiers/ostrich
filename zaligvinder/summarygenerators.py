...
def calculateErrors (track,res):
    ...
    for solver, mres in res.items ():
        ...
        for i,k in enumerate(mres):
            inst = instances[i]
            if k.result != None and inst.expected != None: #inst.expected != None and k.result != None:
...
def terminalResult (track,res):
    ...
    for n in res.keys():
        ...
        table.append ([n,sat,nsat,unk,to,t/1000])
    print(tabulate.tabulate(table,["Solver", "Sat", "Unsat", "Unknown", "Timeout", "Total Time(s)"]))
