...
class ResultRepository:
    ...
    def getAllSatFilesForSolver(self, solver):
        query = '''SELECT TrackInstance.filepath FROM Result,TrackInstance WHERE solver = ? and Result.result IS NOT NULL AND Result.result = true AND Result.instanceid = TrackInstance.id ORDER BY time ASC '''
        rows = self._db.executeRet (query,(solver,))
        return [t[0] for t in rows]
