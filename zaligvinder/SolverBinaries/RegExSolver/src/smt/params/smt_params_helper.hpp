...
struct smt_params_helper {
  ...
  static void collect_param_descrs(param_descrs & d) {
    ...
    d.insert("tactic_model_validation", CPK_BOOL, "validate models produced in smt_tactic", "false","smt");
    ...
    d.insert("seq.giveup_point", CPK_UINT, "quit early for z3str4 (0 means ignore)", "0","smt");
    ...
    d.insert("str.regex_automata_construct_linear_length_constraints", CPK_BOOL, "construct length constraints for automata when expected to be linear (Z3str3 only)", "true","smt");
    d.insert("str.regex_automata_construct_bounds", CPK_BOOL, "construct lower/upper bounds for automata (Z3str3 only)", "true","smt");
    ...
    d.insert("str.pre_milliseconds", CPK_UINT, "number milliseconds to try sequence solver for disjunctive queries (Z3str3 only)", "1000","smt");
    d.insert("str.rewriter_tactic", CPK_BOOL, "use global formula rewriter tactic for string fragment (Z3str3 only)", "false","smt");
    d.insert("str.multiset_check", CPK_BOOL, "use the quick multiset check for word equations (Z3str3 only)", "true","smt");
    d.insert("str.tactic", CPK_STRING, "(all) is static selection of all arms; (arr) is the arrangement-based solver; (las) is the length-abstraction-based solver; (z3str4) is the defualt and includes dynamic difficulty estimation, (ml) uses machine learning to perform arm selection", "z3str4","smt");
    ...
    d.insert("str.search_overlaps", CPK_BOOL, "continue searching when overlapping variables are detected instead of skipping the branch and returning UNKNOWN (Z3str3 only)", "true","smt");
    d.insert("str.search_overlaps_milliseconds", CPK_UINT, "number of milliseconds to try searching overlaps with the arrangement solver (Z3str3 only)", "5000","smt");
    d.insert("str.share_constraints", CPK_BOOL, "enable constraint sharing across tactics (Z3str3 only, has no effect outside of portfolio tactic)", "true","smt");
    d.insert("str.fixed_length_preprocessing", CPK_BOOL, "preprocess formulas with abstraction (Z3str3 only)", "false","smt");
    d.insert("str.fixed_length_iterations", CPK_UINT, "maximum number of attempts to try when preprocessing (Z3str3 only)", "5","smt");
    d.insert("str.regex_prefix_heuristic", CPK_BOOL, "enable regex prefix/suffix heuristic (Z3str3 only)", "true","smt");
  ...
  */
  ...
  bool tactic_model_validation() const { return p.get_bool("tactic_model_validation", g, false); }
  ...
  unsigned seq_giveup_point() const { return p.get_uint("seq.giveup_point", g, 0u); }
  ...
  bool str_regex_automata_construct_linear_length_constraints() const { return p.get_bool("str.regex_automata_construct_linear_length_constraints", g, true); }
  bool str_regex_automata_construct_bounds() const { return p.get_bool("str.regex_automata_construct_bounds", g, true); }
  bool str_fixed_length_refinement() const { return p.get_bool("str.fixed_length_refinement", g, false); }
  unsigned str_pre_milliseconds() const { return p.get_uint("str.pre_milliseconds", g, 1000u); }
  bool str_rewriter_tactic() const { return p.get_bool("str.rewriter_tactic", g, false); }
  bool str_multiset_check() const { return p.get_bool("str.multiset_check", g, true); }
  char const * str_tactic() const { return p.get_str("str.tactic", g, "z3str4"); }
  bool str_fixed_length_naive_cex() const { return p.get_bool("str.fixed_length_naive_cex", g, true); }
  bool str_search_overlaps() const { return p.get_bool("str.search_overlaps", g, true); }
  unsigned str_search_overlaps_milliseconds() const { return p.get_uint("str.search_overlaps_milliseconds", g, 5000u); }
  bool str_share_constraints() const { return p.get_bool("str.share_constraints", g, true); }
  bool str_fixed_length_preprocessing() const { return p.get_bool("str.fixed_length_preprocessing", g, false); }
  unsigned str_fixed_length_iterations() const { return p.get_uint("str.fixed_length_iterations", g, 5u); }
  bool str_regex_prefix_heuristic() const { return p.get_bool("str.regex_prefix_heuristic", g, true); }
