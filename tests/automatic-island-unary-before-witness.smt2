...
; The paired source encodes ((a,a) | (b,a) | (c,c))*.  Restricting the
; second projection to c* therefore implies that the first projection is c*.
; That unary consequence lets the ordinary replace_all rules prove that
; expanded belongs to (cc)*.  The paired image itself is not synchronized:
; replace_all grows only its first tape.
